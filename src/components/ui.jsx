...
export function Button({
  ...
  if (to)   return <Link to={to} className={`${cls} ${className}`} {...rest}>{inner}</Link>;
  if (href) return <a href={href} target="_blank" rel="noopener noreferrer" className={`${cls} ${className}`} {...rest}>{inner}</a>;
  return <button className={`${cls} ${className}`} {...rest}>{inner}</button>;
}
 ...
 */
export function SectionHeader({ label, title, subtitle, theme = "dark", className = "" }) {
  ...
  return (
    <div className={`text-center max-w-2xl mx-auto ${className}`}>
      ...
      {subtitle && <p className={`mt-4 text-base leading-relaxed ${subCls}`}>{subtitle}</p>}
    </div>
  );
}
...
export function StatCard({ value, label, theme = "dark" }) {
  ...
  return (
    <div className="text-center">
      <p className={`font-display text-3xl md:text-4xl font-bold ${valueCls}`}>{value}</p>
...
export function CheckItem({ children, theme = "dark" }) {
  return (
    <li className="check-item">
      ...
      />
      <span className={theme === "light" ? "text-white/80" : "text-stone-600"}>{children}</span>
    </li>
  );
}
